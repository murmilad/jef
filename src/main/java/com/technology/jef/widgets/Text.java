		...
		public Tag assembleTag(String name, TagGenerator generator) {
			...
			Tag elementInput = parrent.add(Tag.Type.INPUT, new HashMap<Tag.Property, String>(){{
				 ...
				 put(Tag.Property.CHANGE, !"".equals(getInputRegexp())
						 ? "if ($(this).val() && !($(this).val().match(/^" + getInputRegexp() + "$/))) { alert(\""+ CurrentLocale.getInstance().getTextSource().getString("data_is_incorrect") +" \\'\" + $(this).val() + \"\\'\"); $(this).val(\"\").change().attr( \"class\", \"error error_color\").trigger(\"refresh\").focus(); return false;} else {$(this).attr( \"class\", \"\").trigger(\"refresh\")}" 
						 : ""
				);
